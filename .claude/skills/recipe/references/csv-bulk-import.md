    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!row.ndisNumber || !/^\d{9}$/.test(row.ndisNumber)) {
        results.push({
          row: rowNum,
          status: 'failed',
          reason: `Invalid NDIS number: "${row.ndisNumber}"`,
        });
        continue;
      }
      if (!row.dateOfBirth || isNaN(Date.parse(row.dateOfBirth))) {
        results.push({
          row: rowNum,
          status: 'failed',
          reason: `Invalid date of birth: "${row.dateOfBirth}"`,
        });
        continue;
      }
...
const effectiveCompanyId =
  req.user?.role === 'admin' ? (req.body.companyId as string) : req.user?.companyId; // non-admin always uses their own company

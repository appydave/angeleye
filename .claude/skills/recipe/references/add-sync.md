...
export function resolveConflict(
  ...
  return withGitLock(async (): Promise<GitResolveResult> => {
    ...
    if (remaining === 0) {
      ...
      try {
        await git(['rebase', '--continue'], 30_000);
      } catch {
        /* may need more resolves */
      }
    }
...
export async function getFolderSyncStatus(): Promise<FolderSyncStatus> {
  const relayExists = await fs.access(RELAY_ROOT).then(
    () => true,
    () => false
  );
...
async function detectChanges(
  ...
  try {
      ...
      try {
        ...
        if (sourceStat.mtimeMs > targetStat.mtimeMs) {
          changes.push({
            ...
            modified: sourceStat.mtime.toISOString(),
          });
        }
      } catch {
        changes.push({
    ...
    /* source dir empty or unreadable */
  }
...
import {
  ...
  acceptIncoming,
  acceptAllIncoming,
} from '../services/folder-sync.service.js';
import { apiSuccess, apiFailure } from '../helpers/response.js';
